...
import net.hznu.common.plugin.IPlugin;
...
import net.hznu.common.plugin.PluginMaster;
...
public class Setup implements org.nutz.mvc.Setup {
    ...
    private void initSysData(NutConfig config, Dao dao) {
        ...
        if (0 == dao.count(Sys_user.class)) {
            ...
            conf.setConfigValue("教育现代化水平监测");
            conf.setNote("系统名称");
            dao.insert(conf);
            conf = new Sys_config();
            conf.setConfigKey("AppShrotName");
            conf.setConfigValue("eduevaluate");
            conf.setNote("系统短名称");
            dao.insert(conf);
            conf = new Sys_config();
            ...
            unit.setLocation(0);
            unit.setAddress("杭州市余杭区");
            unit.setEmail("71173618@qq.com");
            unit.setTelephone("");
            unit.setHasChildren(false);
            unit.setParentId("");
            unit.setWebsite("http://jyxdhpg.cn");
